...
const tooltipStyle = {
  ...
  labelStyle: { color: "oklch(0.65 0.015 240)", fontSize: 10, textTransform: "uppercase" as const, letterSpacing: "0.1em" },
};
  ...
  return (
    <div className="h-full overflow-y-auto">
      ...
      <div className="grid grid-cols-12 gap-3 sm:gap-4 p-4 sm:p-6">
        ...
        <Insight tone="red" label="Critical surge" value="3 zones" change="trending" icon={TrendingUp} />
          ...
          {isLoading ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={hourlyTrend}>
                ...
                <XAxis dataKey="hour" stroke="oklch(0.55 0.015 240)" tick={{ fontSize: 10 }} interval={2} />
                <YAxis stroke="oklch(0.55 0.015 240)" tick={{ fontSize: 10 }} />
                <Tooltip {...tooltipStyle} />
                <Area type="monotone" dataKey="events" stroke="#bef264" strokeWidth={2} fill="url(#g1)" />
                <Area type="monotone" dataKey="critical" stroke="#ff4d4d" strokeWidth={1.5} fill="url(#g2)" />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </Card>
        ...
        <Card span={4} title="Priority Mix" sub="Active distribution">
          {isLoading ? (
            ...
            <>
              <ResponsiveContainer width="100%" height={260}>
                <RadialBarChart innerRadius="35%" outerRadius="100%" data={priorityBreakdown.map((p) => ({ ...p, fill: priorityHex(p.priority) }))} startAngle={90} endAngle={-270}>
                  <PolarAngleAxis type="number" domain={[0, Math.max(...priorityBreakdown.map((p) => p.count), 1)]} tick={false} />
                  <RadialBar background={{ fill: "oklch(0.18 0.01 240)" }} dataKey="count" cornerRadius={6} />
                  <Tooltip {...tooltipStyle} />
                </RadialBarChart>
              </ResponsiveContainer>
              <div className="grid grid-cols-2 gap-1 px-1 pb-2 text-xs">
                {priorityBreakdown.map((p) => (
                  <div key={p.priority} className="flex items-center gap-1.5">
                    <span className="h-2 w-2 rounded-full" style={{ background: priorityHex(p.priority) }} />
          ...
          {isLoading ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={causeBreakdown} layout="vertical" margin={{ left: 20 }}>
                ...
                <YAxis dataKey="cause" type="category" stroke="oklch(0.55 0.015 240)" tick={{ fontSize: 11 }} width={120} />
                <Tooltip {...tooltipStyle} />
              ...
              {zoneIntelligence.map((z) => (
                <div key={z.zone} className="rounded-xl border border-border bg-surface-elevated/40 p-3">
                  ...
                  <div className="mt-1 flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <span className="font-mono font-semibold" style={{ color: z.risk > 70 ? "#ff4d4d" : z.risk > 40 ? "#ff9533" : "#bef264" }}>
                      {z.risk}
                    </span>
                  </div>
                  <div className="mt-2 h-1 overflow-hidden rounded-full bg-muted">
                    <div className="h-full rounded-full" style={{ width: `${z.risk}%`, background: z.risk > 70 ? "#ff4d4d" : z.risk > 40 ? "#ff9533" : "#bef264" }} />
                  </div>
                </div>
              ))}
            </div>
...
function Card({ span, title, sub, children }: { span: number; title: string; sub: string; children: React.ReactNode }) {
  return (
    <div className={`col-span-12 lg:col-span-${span} rounded-2xl border border-border bg-surface/60 p-5 backdrop-blur-xl`}>
